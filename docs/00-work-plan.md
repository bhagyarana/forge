...
| **Ph2** Explorer                   | 🔄 in progress — see 4.2 below. Three real bugs found and fixed on 2026-09-05 while validating against a login-gated target; see the Ph2 addendum in [TASKLIST.md](../TASKLIST.md).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **Ph3**, **Ph4**, **Ph6**          | 🔄 **demo fast-track only, 2026-09-05** — none of the phase checkpoints below are started (no tests-first, no agentic call sites). A separate, deterministic-only vertical slice was built end to end instead — `pnpm forge demo <url>` — to have something real to show on short notice. See the scope note at the top of each phase in [TASKLIST.md](../TASKLIST.md) for exactly what that command does and does not cover.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Ph5** Triage + Healer            | ✅ **built for real, 2026-09-05**, superseding its own demo fast-track — test-first, all seven checkpoints (Ph5.1–Ph5.7) green. The full ten-row/six-cause pre-classifier, the exact six-signal scorer and ten-rung ladder, all five vetoes, both decision gates, patch/verify/rollback, the repeat-failure cache, and both agent call sites (`packages/agents/triage`) with the `client: null` deterministic default `FR-605` requires. `packages/orchestrator/src/triage-heal.ts` drives the real FSM/guards through the real store and is wired into `scheduler.ts`'s `RUNNING` branch. 111 new tests, all green under `pnpm verify`. Two named scope cuts carried forward (not reopened): no `apps/sut` golden fixtures for `EC-04`/`EC-05`/`EC-06` (reproduced instead as orchestrator-level integration tests with the same traces); the Runner still doesn't emit typed `ToolError`s from live failures (`Ph4`'s job — `demo.ts` bridges this with a narrow, named shim). See the Ph5 note in [TASKLIST.md](../TASKLIST.md) for the full account. |
| **Ph7** Design & Psychology Critic | ✅ **built, 2026-09-05** — reinstates [ADR-013](decisions/ADR-013-design-intelligence-deferred.md)'s deferred design-intelligence pillar per its own flip trigger, extended with growth.design's psychology cheatsheet, case studies, and CLEAR framework ([ADR-018](decisions/ADR-018-design-psychology-critic-reinstated.md), [docs/07-design-psychology/](07-design-psychology/)). Ten pure checks, test-first (25 unit tests), a five-component Delight Score, a human-friendly grouped narrative report, and live screenshot streaming in `forge ui`. Additive and non-gating — see the [TASKLIST.md Ph7 note](../TASKLIST.md) for what shipped and what's deliberately deferred.                                                                                                                                                                                                                                                                                                                                                                   |
...
### 4.2 Ph2 status — what landed and what is deliberately deferred

Built, tested, green under `pnpm verify`: `packages/perception` (snapshot tree, `stateSignature()`, `affordancesOf()`, the destructive deny-list, `detectLoginForm()`, `authOutcome()` — all pure, plus the one live `captureSnapshot()` seam), three login-page fixtures (`fixtures/perception/*.snapshot.yaml`), `packages/core/src/prioritise.ts` (`cluster()`'s five passes, six-factor `rank()`, wired into `orchestrator/guards.ts`'s `rankCapabilities`, replacing the Ph1 stand-in sort), and `packages/agents/explorer` (the frontier loop, `authenticate()`, the deterministic value-sort fallback, a `PlaywrightDriver` for real targets and a `FakeDriver` for tests). `forge explore <url>` runs against a real target (verified live against `https://httpbin.org/` and `https://example.com/`).

**Deliberately out of scope for this pass** (agreed with the user before starting): `apps/sut`, the reference-shop target app the live-tier golden cases (`EC-01`…`EC-07`) run against, does not exist yet, so none of those golden cases can run live. Concretely still open before Ph2 can close:

- `fixtures/golden/EC-02.json` and the eval-harness support for a non-session ("`forge explore`, not a full session") case shape — today's `packages/evals` runner only drives full sessions through the API.
- `storageState` persistence (`.auth/state.json`, written once and reused — `FR-102`) — `authenticate()` detects and drives the login flow correctly, but nothing yet writes the captured state to disk or reuses it.
- The politeness throttle between navigations (09 §3).
- Wiring call site 1 (`chooseBatch`) to `runAgentLoop()` from `packages/agents/harness` — the seam (`ExploreDeps.chooseBatch`) exists and the deterministic fallback is the default and is what is tested, but the actual model-backed implementation is unwritten.
- `packages/orchestrator/src/scheduler.ts` still calls `stubExplore()`, not the real `explore()` — wiring a live browser into the session FSM is follow-on work, not attempted here to avoid destabilising the frozen Ph1 spine.

One documentation discrepancy surfaced while testing against the worked example in [09 §6.3](03-algorithms/09-exploration-and-prioritisation.md): recomputing that table's six risk scores from its own stated formula reproduces the two rows with `authProximity = 0` exactly (Cart 0.446, Browse & Search 0.220) but not the other three, which are each too high by ~0.10 — a transcription slip in the doc, not a hint that `packages/core/src/prioritise.ts`'s formula is wrong (see the comment above `prioritise.test.ts`'s worked-example test, which asserts the corrected numbers and confirms the claimed backlog order is unaffected). Left as-is pending a maintainer decision on 09 §6.3's text.
